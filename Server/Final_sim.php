...
QUERY;
            ...
            if ($server_response["current_round"]["bonus_games_won"]) {
                ...
                if (array_key_exists("bonus_game_id", $resp[0]) && $resp[0]['bonus_game_id'] == 380001) {
                    ...
                    $fMul = $server_response["current_round"]["multiplier"];
                    $this->bonusBaseGameOcc++;
                    if ($fsMul > 0 ) {
                        $this->postInfoSekh["medua_Base_mul"]["mul " . $fMul] += 1;
                        $this->bonusBaseWinOcc++;
                        $this->bonusBaseGameWin += $fsMul;
                    } 
                } elseif($resp[0]['feature_name'] == "BaseGameFeature"){
                    // echo "base";
                    $this->mysteryFeatureOcc++;
                    $this->mysterSymbolOccurence[$resp[0]["mystry_symbol"]]++;
                    while($server_response["current_round"]["bonus_games_won"]!=""){
                        $request_params_sim['game_id'] = $this->game_id;
                        $request_params_sim['amount_type'] = 1;
                        $request_params_sim['sub_game_id'] = 4;
                        $request_params_sim['coin_value'] = $this->coin_value_sim;
                    ...
                    $this->mystFeatureWin += $server_response["current_round"]["total_fs_win_amount"];
                } elseif($resp[0]['feature_name'] == "LightningFeature"){
                    $this->lightingFeatureOcc++;
                    // $this->total_normal_win_occ += 1;
                    // $this->total_normal_win += $server_response["current_round"]["win_amount"];
                    $oneRound = false;
                    while($server_response["current_round"]["bonus_games_won"]!=""  && $server_response["current_round"]["bonus_games_won"][0]["type"]=="respins"){
                        $request_params_sim['game_id'] = $this->game_id;
                        $request_params_sim['amount_type'] = 1;
                        $request_params_sim['sub_game_id'] = 2;
                        $request_params_sim['coin_value'] = $this->coin_value_sim;
                        ...
                        if($server_response["current_round"]["misc_prizes"] != "" ){
                            if($server_response["current_round"]["bonus_games_won"]== "" || array_key_exists('type', $server_response["current_round"]["bonus_games_won"][0]) && $server_response["current_round"]["bonus_games_won"][0]['type'] != "freespins"){
                                if($oneRound == false){
                                    $s = substr_count($server_response["current_round"]['matrix'],"s");
                                    $z = substr_count($server_response["current_round"]['matrix'],"z");
                                    // echo "count =  ".$s+$z."running" ;
                                    $this->total_scatter_occ["one_respin"][$s+$z] += 1;
                                    $oneRound =true; 
                                }
                            }
                        }
                        
                        $this->lightFeatureWin += $server_response["current_round"]["win_amount"];
                    }                    
                    if ($server_response["current_round"]["bonus_games_won"]!="" && array_key_exists('type', $server_response["current_round"]["bonus_games_won"][0])) {
                        
                        if ($server_response["current_round"]["bonus_games_won"][0]['type'] == "freespins") {
                            $z = substr_count($server_response["current_round"]['matrix'],"z");
                            $this->total_scatter_occ["freespin"][$z] += 1;
                            $this->total_scatter += 1;
                            $num_spins = $server_response["current_round"]["bonus_games_won"][0]['num_spins'];
                            for ($j = 0; $j < $num_spins; $j++) { 
                                ...
                                $res = s_main($request_params);
                                $this->total_free_game_win+=$server_response["current_round"]["win_amount"];
                                if($server_response["current_round"]!="" && $server_response["current_round"]["sctter_win"] > 0){
                                    $this->additionalSpins++;
                                    $this->total_scatter_occ["ad_freespin"][$server_response["current_round"]["sctter_win"]] += 1;
                                    $num_spins = $server_response["current_round"]["num_spins"];
                                }
                                }
                        }   
                    }
                    ...
                    if ($resp[0]['type'] == "freespins") {
                        ...
                        for ($j = 0; $j < $num_spins; $j++) {
                            ...
                            if (isset($resp_b) && isset($resp_b[0]) && array_key_exists("bonus_game_id", $resp_b[0]) && $resp_b[0]['bonus_game_id'] == 380004){
                                ...
                                if ($fsMul > 0 ) {
                                    $this->postInfoSekh["medua_FG_mul"]["mul " . $fMul]+= 1;
                                    $this->bonusFreeWinOcc++;
        ...
        elseif($this->game_id == 710){
            $game_detail = array(
                        "total baseGameFeature Occurence"=> $this->mysteryFeatureOcc ,
                        "baseGameFeature Win:"=> $this->mystFeatureWin,
                        "baseGameFeature RTP:"=> round(($this->mystFeatureWin / $this->total_bet_simulation) * 100, 2),
                        "baseGameFeatureSymbol Occurence:"=> $this->mysterSymbolOccurence,
                        "lighting feature win:"=> $this->lightFeatureWin,
                        "lighting feature occ:"=> $this->lightingFeatureOcc,
                        "lighting feature rtp :"=> round(($this->lightFeatureWin / $this->total_bet_simulation) * 100, 2) 
                            );
                        }
        elseif($this->game_id == 707){
            $game_detail = array(
                "option1_occ"=> $this->option1_occ,
                "option2_occ"=> $this->option2_occ,
                "option3_occ"=> $this->option3_occ,
