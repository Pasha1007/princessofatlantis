...
class olympus_bonazaV2
{
    ...
    $thumbletotalOcc, $thumbleOcc, $total_scatter_win1, $total_scatter_win2, $fg_count, $thumbleOccFG, $thumbletotalOccFG, $multiplierCount;
    #====================================================
...
QUERY;
        ...
        // $this->min_coins = 1;
        // $this->max_coins = 1;
        $this->coin_value_sim = 20;  // buy,fg , normal
        // $this->coin_value_sim = 28; // ante
        $timeCount = 0;
...
QUERY;
            ...
            // $this->total_bet_simulation += 28; // normal/ ante
                ...
                if (array_key_exists('type', $resp_fg[0])) {
                    if ($resp_fg[0]['type'] == "freespins") {
                        ...
                        for ($j = 0; $j < $num_spins; $j++) {
                            ...
                            $request_params_sim['game_id'] = $this->game_id;
                            $request_params_sim['amount_type'] = 1;
                            // $request_params_sim['sub_game_id'] = 3; // ante, normal
                            ...
                            if($server_response["current_round"]["post_matrix_info"]){
                                $resp_fg = $server_response["current_round"]["post_matrix_info"];
        ...
        foreach ($this->repeat_num_Normal as $key => $value) {
            ...
            $num_prize = $payta[$key];
            $total_val[$key] = $value * $num_prize * $this->coin_value_sim * $this->num_coins_sim;
        }
        $total_val_rtp = array();
        $total_sym_rtp = 0;
        foreach ($total_val as $key => $value) {
            $total_val_rtp[$key] = round(($value / $this->total_bet_simulation) * 100, 2);
            $total_sym_rtp += round(($value / $this->total_bet_simulation) * 100, 2);
        }
        ...
        $print4 = json_encode(array("Total Cycle" => $this->cycle ,
            ...
            "Base symbol win. "=> $total_val ,
            "Base each symbol rtp . "=> $total_val_rtp ,
            "Base total symbol Rtp. "=> $total_sym_rtp ,
