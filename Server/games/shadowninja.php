...
class shadowninja
{
    ...
    $total_sub_game_occ, $MystryFeature, $StickyFeature,$MystryFeatureFG, $StickyFeatureFG, $fg1, $fg2,$total_free_game2, $total_free_game_win2, $num_fs_arr;
    #====================================================
...

    {
        ...
        $this->total_free_game2 = 0;
        $this->total_free_game_win2 = 0;
        $this->total_respin = 0;
        ...
        $this->MystryFeature = 0;
        $this->StickyFeature = 0;
        $this->MystryFeatureFG = 0;
        $this->StickyFeatureFG = 0;
        $this->fg1 = 0;
        $this->fg2 = 0;
        $this->slag = array();
        $this->round_case = array();
        $this->num_fs_arr = array("8"=>0,"10"=>0,"12"=>0);
        ...
        $this->total_sub_game_occ = array("base_game"=>array("3"=>0,"4"=>0,"5"=>0),"base_1"=>array("3"=>0,"4"=>0,"5"=>0),"base_2"=>array("3"=>0,"4"=>0,"5"=>0),"base_3"=>array("3"=>0,"4"=>0,"5"=>0));
        // $this->$finalWin = array();
    ...
    public function handleSpin($requestParams)
    {
        ...
        $this->total_scatter_occ = array("base_game"=>array("3"=>0,"4"=>0,"5"=>0),"base_1"=>array("3"=>0,"4"=>0,"5"=>0),"base_2"=>array("3"=>0,"4"=>0,"5"=>0),"base_3"=>array("3"=>0,"4"=>0,"5"=>0),"fg_occ"=>array());
        $this->total_sub_game_occ = array("base_game"=>array("3"=>0,"4"=>0,"5"=>0),"base_1"=>array("3"=>0,"4"=>0,"5"=>0),"base_2"=>array("3"=>0,"4"=>0,"5"=>0),"base_3"=>array("3"=>0,"4"=>0,"5"=>0),"fg_occ"=>array());
        $this->repeat_num_Normal = array(
            "5w" => 0,
            "4w" => 0,
            "3w" => 0,
...
QUERY;
            ...
            //  $this->total_bet_simulation += $this->coin_value_sim *$this->num_coins_sim *80;// buyfg1 
            // $this->total_bet_simulation += $this->coin_value_sim *$this->num_coins_sim *200;// buyfg2
            #=========================================================
            $request_params_sim['game_id'] = $this->game_id;
            $request_params_sim['amount_type'] = 1;
            $request_params_sim['request_type'] = 2;
            $request_params_sim['sub_game_id'] = 13;
            // $request_params_sim['request_type'] = 8; // buyfg
            // $request_params_sim['sub_game_id'] = 12; // buy fg1
            // $request_params_sim['sub_game_id'] = 6; // buy fg2
            ...
            if($server_response["current_round"]["post_matrix_info"] and isset($server_response["current_round"]["post_matrix_info"]["feature_name"])){
                if($server_response["current_round"]["post_matrix_info"]["feature_name"] == "MystryFeature"){
                    $this->MystryFeature += 1;
                }
                if($server_response["current_round"]["post_matrix_info"]["feature_name"] == "StickyWild"){
                    $this->StickyFeature +=1;
                }
            }
            //MystryFeature
            $this->total_scatter_occ["base_game"][$server_response["current_round"]['scattersCount']['s']] += 1; //775
            $this->total_sub_game_occ["base_game"][$server_response["current_round"]['post_matrix_info']['reel_pos']] += 1; //775
            
            
            // $server_response["current_round"]["bonus_games_won"] = [];
            ...
            if ($server_response["current_round"]["bonus_games_won"]) {
                ...
                if ($resp[0]['type'] == "freespins") {
                    if ($resp[0]['feature_name'] == "fg1") {
                        $type = "fg1";
                        $this->fg1 += 1;
                        $this->total_free_game += 1;
                        
                    }
                    if ($resp[0]['feature_name'] == "fg2") {
                        $type = "fg2";
                        $this->fg2 += 1;
                        $this->total_free_game2 += 1;
                    }

                        $total_fs_win = 0;
                        $num_s = $resp[0]['num_spins'];                   
                        $this->total_scatter_occ["fg_occ"][$num_s] += 1; //775
                        $fs = true;
                        // $c = 0;
                        ...
                        while ($fs == true ) {
                            $request_params_sim['game_id'] = $this->game_id;
                            $request_params_sim['amount_type'] = 1;
                            $request_params_sim['sub_game_id'] = 16;
                            $request_params_sim['username'] = $this->username_sim;
                            $request_params_sim['coin_value'] = $this->coin_value_sim;
                            $request_params_sim['num_coins'] = $this->num_coins_sim;
                            $request_params_sim['num_betlines'] = $this->num_betlines_sim;
                            ...
                            // print_r($server_response);
                                if(isset($server_response["current_round"]["post_matrix_info"]["extra_fs"])){
                                    if (count($server_response["current_round"]["post_matrix_info"]["extra_fs"]) == 2){
                                        $this->num_fs_arr[12] +=1;
                                    }
                                    elseif (count($server_response["current_round"]["post_matrix_info"]["extra_fs"]) == 1){
                                        $this->num_fs_arr[10] +=1;
                                    }
                                    elseif (count($server_response["current_round"]["post_matrix_info"]["extra_fs"]) == 0){
                                        $this->num_fs_arr[8] +=1;
                                    }
                                }
                            }
                            // if(isset($server_response["next_round"]["spins_left"]) and $server_response["next_round"]["spins_left"] == 1){
                            //     // $server_response["next_round"]["spins_left"];
                            //     $this->num_fs_arr[$server_response["next_round"]['num_spins']] +=1;
                            //     $this->num_fs_arr[$server_response["next_round"]['num_spins']] +=1;
                            // }
                        // print_r($server_response["current_round"]);
                        if($server_response["current_round"]["post_matrix_info"] and isset($server_response["current_round"]["post_matrix_info"]["feature_name"])){
                            if($server_response["current_round"]["post_matrix_info"]["feature_name"] == "MystryFeature"){
                                $this->MystryFeatureFG += 1;
                            }
                            if($server_response["current_round"]["post_matrix_info"]["feature_name"] == "StickyWild"){
                                // print_r($server_response["current_round"]);
                                $this->StickyFeatureFG +=1;
                            }
                        }                   
                        // print_r($server_response["current_round"]);
                        $total_fs_win += $server_response["current_round"]["win_amount"];
                    }
                    if($type == "fg1"){
                        
                        $this->total_free_game_win += $total_fs_win;
                    }
                    if($type == "fg2"){
                        
                        $this->total_free_game_win2 += $total_fs_win;
                    }
        ...
        $this->total_win_simulation = $this->total_normal_win + $this->total_free_game_win + $this->total_free_game_win2+$this->total_free_game_respin_win + $this->total_respin_win +
        ...
        $print4 = json_encode(array("Total Cycle" => $this->cycle ,
            ...
            "Free Game2 Hit freq. "=> round(($this->total_free_game2 / $this->cycle) * 100, 2) ,
            ...
            "total_sub_game_occ"=>  $this->total_sub_game_occ ,
            "Base Win"=>  $this->total_normal_win ,
            "Total FG Win"=>  $this->total_free_game_win ,
            "Total FG2 Win"=>  $this->total_free_game_win2 ,
            "Total Win "=> $this->total_win_simulation ,
            "num fs csme "=> $this->num_fs_arr ,
            "FG Came "=> $this->total_free_game ,
            "FG2 Came "=> $this->total_free_game2 ,
            "BAse Came "=> $this->total_normal_win_occ ,
            "Base RTP "=> round(($this->total_normal_win / $this->total_bet_simulation) * 100, 2) ,
            "BMystryFeature Occ "=> $this->MystryFeature ,
            "Sticky feature occ"=> $this->StickyFeature ,
            "FG BMystryFeature Occ "=> $this->MystryFeatureFG ,
            "FG Sticky feature occ"=> $this->StickyFeatureFG ,
            "FG1 Occ"=> $this->fg1,
            "FG2 Occ"=> $this->fg2 ,

            "FG RTP "=> round(($this->total_free_game_win / $this->total_bet_simulation) * 100, 2) ,
            "FG2 RTP "=> round(($this->total_free_game_win2 / $this->total_bet_simulation) * 100, 2) ,
            "Total FG Occ / Scatter Occ"=>  $this->total_scatter_occ ,
            "Total SCatter Occ "=>  $this->total_scatter ,
