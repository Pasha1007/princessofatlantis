...
QUERY;
    ...
    if ($db->numRows($re) > 0) {
        ...
        while ($row1 = $db->fetchAssoc($re)) {
                $game_fs["game_name"] = $row1['game_name'];
                $game_fs["game_id"]=$row1['game_id'];
                $game_fs["sub_game_id"]= 1;
                // $game_fs["gameName "]= $row1['game_name'];
                $game_fs["title"]= $row1['title'];
                $game_fs["description"]= $row1['description'];
                $game_fs["num_rows"]= $row1['num_rows'];
                $game_fs["num_columns"]= $row1['num_columns'];
                $game_fs["min_coins"]= $row1['min_coins'];
                $game_fs["max_coins"]= $row1['max_coins'];
                $game_fs["default_coins"]= $row1['default_coins'];
                $game_fs["default_coin_value"]= $row1['default_coin_value'];
                $game_fs["paylines"]= $row1['paylines'];
                $game_fs["coin_values"]= $row1['coin_values'];
                $game_fs["elements"]= $row1['elements'];
                $game_fs["wilds"]= $row1['wilds'];
                $game_fs["bonus"]= $row1['bonus'];
                $game_fs["scatters"]= $row1['scatters'];
                $game_fs["reelset_config"]= $row1['reelset_config'];
                $game_fs["pjp_config"]= $row1['pjp_config'];
                $game_fs["unity_symbols"]=  $row1['unity_symbols'];
                $game_fs["wild_multiplier"]= $row1['wild_multiplier'];
                $game_fs["left2right"]= $row1['left2right'];
                $game_fs["right2left"]= $row1['right2left'];
                $game_fs["enabled"]= $row1['enabled'];
                $game_fs["fixed_coins"]= $row1['fixed_coins'];
                $game_fs["paylines_type"]= $row1['paylines_type'];
                $game_fs["spin_type"]= $row1['spin_type'];
                $game_fs["category_id"]= $row1['category_id'];
                $game_fs["misc"]= $row1['misc'];
                $game_fs["rtp"]= $row1['rtp'];
                $game_fs["bonus_config"]= "";
                $game_fs["symbol_config".$row1['sub_game_id']]= $row1['symbol_config'];
                $game_fs["reels".$row1['sub_game_id']]= $row1['reels'];
                $game_fs["bonus_config".$row1['sub_game_id']]= $row1['bonus_config'];
                $game_fs["pay_table" .$row1['sub_game_id']]= $row1['pay_table'];
...
QUERY;
        ...
        $this->num_betlines_sim = $game_fs['default_coins'];
        $this->num_row = $game_fs['num_rows'];
        $this->num_col = $game_fs['num_columns'];
        ...
        $request_params_sim['request_type'] = 2; //buy fg 
        $request_params_sim['sub_game_id'] = 1; // buy fg
        // $request_params_sim['sub_game_id'] = 5;  // nate 
        // $request_params_sim['coin_value'] = 20; // base
        // $request_params_sim['coin_value'] = 20; // ante
        $request_params_sim['coin_value'] = $this->coin_value_sim*100; //  buy fg
        $request_params_sim['coin_value2'] = 1; // norma, buy fg
...
QUERY;
            ...
            // $this->total_bet_simulation += 25; // ante
            ...
            $request_params_sim['sub_game_id'] = 3; // buy fg
            // $request_params_sim['sub_game_id'] = 5;  // nate 
            // $request_params_sim['coin_value'] = 20; // ante
            ...
            $request_params_sim['num_betlines'] = $this->num_betlines_sim;
            $request_params_sim['account_id'] = $this->account_id;
            $request_params_sim['username'] = "pg9";
            $request_params_sim['session_id'] = "";
            $request_params_sim['platform_type'] = "desktop";
            #====================================================
            $_POST = $request_params_sim;
            ...
            if($server_response["current_round"]["misc_prizes"]){
                ...
                if(array_key_exists("count",$resp_misc ) and  $resp_misc["count"] > 0 )
                {
                    $this->thumbleOcc+=1;
            ...
            if ($server_response["current_round"]["bonus_games_won"]) {
                ...
                if (array_key_exists('type', $resp_fg[0])) {
                    if ($resp_fg[0]['type'] == "freespins") {
                        $this->total_scatter_win += $server_response["current_round"]["scatter_win"] ; // ANte
                        ...
                        while ($fs == true ) {
                            ...
                            $request_params_sim['game_id'] = $this->game_id;
                            $request_params_sim['amount_type'] = 1;
                            $request_params_sim['sub_game_id'] = 4; // , normal
                            // $request_params_sim['sub_game_id'] = ; // buy fg
                            // $request_params_sim['sub_game_id'] = 6; // ante,
                            // $request_params_sim['coin_value'] = 20; // ante
                            ...
                            // print_r($server_response["current_round"]["post_matrix_info"]);
                            // if($server_response["current_round"]["post_matrix_info"]){
                        ...
                        if($this->total_free_game_win >$this->freeGamebaseWin ){
                        $this->freeGamebaseWin = $this->total_free_game_win;
                        }
                        }
                        }
        ...
        $print4 = json_encode(array("Total Cycle" => $this->cycle ,
            ...
            " highest win"=> $this->freeGamebaseWin  ,
