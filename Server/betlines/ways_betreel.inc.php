...
class WaysBetreel implements iBetLines
{
    private $game;
    private $round;
    ...
    public function generateBetLines()
    {
        ...
        for($i = 0; $i < $cols; $i++) {
            ...
            for($j = 0; $j < $rows; $j++, $k++) {
                $reverseMatrix[$j][$i] = $elementMatrix[$j][$i];
            }
        }
        // print_r($reverseMatrix);
        $this->round->matrix = $reverseMatrix;
        $this->round->matrixString = array2d_to_string($this->round->matrix);
		$this->round->generateMatrixData($this->round->matrix);
		$this->round->matrixFlatten = $this->round->generateFlatMatrix($this->round->matrix);
        $this->round->postMatrixInfo['matrixString'] = array2d_to_string($this->round->matrix); // TODO
        ...
        $this->round->numBetLines = count($betlines['bet_strings']);
    ...
    public function reelConfig($reel_number) {
        $spin_type = $this->round->spinType;
        $conf = $this->game->symbolConfig;
        if( isset($conf['reelConfig'][$spin_type]) && isset($conf['reelConfig'][$spin_type]['feature']) ) {
            $reels_length = $conf['reelConfig'][$spin_type]['feature'];
