...
gWV.addGameElements = function () {
  ...
  _mediator.subscribe(
    "removeAllmultipliers",
    this.removeAllmultipliers.bind(this)
  );
  ...
  _mediator.subscribe(
    "updateTextWithCountUp",
    this.updateTextWithCountUp.bind(this)
  );
  ...
  if (this.BonusPos.length > 0) {
    setTimeout(
      function () {
        ...
        if (
          !(
            coreApp.gameModel.userModel.userData.next_round &&
            coreApp.gameModel.userModel.userData.next_round.type == "freespins"
          )
        )
          _mediator.publish("EnablePanel");
        _mediator.publish("FadeReels", 0.3);
      }.bind(this),
      _ng.isQuickSpinActive && _ng.quickSpinType === "turbo"
        ? 500
        : this.BonusPos.length * 450
    );
  } else {
    this.showLine();
  }
...
gWV.showAllWins = function (allWinLines) {
  ...
  for (let k = 0; k < this.linesObj.length; k++) {
    ...
    for (var i = 0; i < this.linesObj[k].pos.length; i++) {
      ...
      if (
        isWildInWinLine &&
        _ng.GameConfig.symbolAnimations[symbol + "_wild"]
      ) {
        symbol = symbol + "_wild";
      }
      ...
      if (
        this.animationConfig[symbol] &&
        this.animationConfig[symbol][0].offset &&
        _viewInfoUtil.isSecondaryAssetsLoaded
      ) {
        symbolOffset = this.animationConfig[symbol][0].offset;
      }
      var xReelPos = this.getSymbolFromPosition(
        this.linesObj[k].pos[i],
        "xValue"
      );
      var yReelPos = this.getSymbolFromPosition(
        this.linesObj[k].pos[i],
        "yValue"
      );
      winSym.x = this.reelConfig.data.eachReelPos[xReelPos] + symbolOffset.x;
      var symbolTopPadding = this.reelConfig.data.eachReelYPos
        ? this.reelConfig.data.eachReelYPos[xReelPos]
        : 0;
      var yPos = Math.floor(
        Number(this.linesObj[k].pos[i]) / Number(this.reelConfig.data.noOfReels)
      );
      var gaps = this.symConfig.symbolYGap * (yPos + 1);
      winSym.y =
        this.symConfig.symbolHeight * (yPos + 1) +
        gaps +
        symbolTopPadding +
        symbolOffset.y;
      if (this.getValidateExpWild(this.linesObj[k].pos[i])) {
        ...
        expWild.x = winSym.x - 92;
        expWild.y = winSym.y - 89;
        expWild.play();
...
gWV.removeMultipliers = function () {
  ...
  this.currentTotal = 0;
  if (
    coreApp.gameModel.userModel.userData.current_round.spin_type == "freespin"
  ) {
    if (this.stickyContainer) {
...
gWV.createSticky = function () {
  if (
    coreApp.gameModel.userModel.userData.current_round.spin_type ==
      "freespin" &&
    this.currentTotal > 0
  ) {
...
gWV.createStickyandTween = function () {
  ...
  for (let index = 0; index < this.symbolIndex.length; index++) {
    ...
    if ((this.TweenMultiplierText, this.symbolIndex[index][0] < 10)) {
      ...
      (this.TweenMultiplierText,
      this.symbolIndex[index][0] >= 10 && this.symbolIndex[index][0] < 20)
    ) {
      ...
      (this.TweenMultiplierText,
      this.symbolIndex[index][0] >= 20 && this.symbolIndex[index][0] <= 50)
    ) {
...
gWV.showEachReelStopAction = function (reelId, reelStrip) {
  ...
  if (
    !_ng.GameConfig.ReelViewUiConfig.showScatterLand ||
    !_viewInfoUtil.isSecondaryAssetsLoaded
  ) {
    return;
  }
...
gWV.tweenAnimation = function () {
  ...
  for (let j = 0; j < this.multiplierArray.length; j++) {
    ...
    if (_viewInfoUtil.viewType == "VD") {
      xPos =
        this.multiplierWinTxt.worldTransform.tx -
        (this.multiplierArray[j][0].worldTransform.tx -
          this.multiplierArray[j][0].x);
      yPos =
        this.multiplierWinTxt.worldTransform.ty -
        (this.multiplierArray[j][0].worldTransform.ty -
          this.multiplierArray[j][0].y);
    } else {
      xPos = 490;
      yPos = 0;
    }
    TweenMax.to(this.multiplierArray[j][0], time, {
      ...
      onComplete: function () {
        this.multiplierArray[j][2].parent.removeChild(
          this.multiplierArray[j][2]
        );
        _sndLib.play(_sndLib.sprite.multi_8);
        this.multiplierArray[j][0].alpha = 0;
        this.multiplierArray[j][0].parent.removeChild(
          this.multiplierArray[j][0]
        );
...
gWV.tweenTotalWin = function (value) {
  ...
  if (
    coreApp.gameModel.obj.current_round.post_matrix_info.win ==
...
gWV.tweenFinal = function () {
  ...
  let xPos =
    this.allWin.worldTransform.tx - this.mText.worldTransform.tx - this.mText.x;
  let yPos =
    this.allWin.worldTransform.ty - this.mText.worldTransform.ty - this.mText.y;
  // if (sum > 0) {
  let time = 0;
  if (_ng.isQuickSpinActive && _ng.quickSpinType === "turbo") {
    time = 0.15;
  ...
  TweenMax.to(this.mText, time, {
    ...
    onComplete: function () {
      ...
      let roundWin =
        coreApp.gameModel.userModel.userData.current_round.payline_win_amount;
      _mediator.publish("updateTextWithCountUp", roundWin, 150, this.allWin);
      if (
        coreApp.gameModel.userModel.userData.current_round.spin_type ==
        "freespin"
      ) {
        this.freespinTotalValue =
          coreApp.gameModel.userModel.userData.current_round.total_fs_win_amount;
      } else {
      ...
      _mediator.publish(
        ...
        winTxt +
          ": " +
          pixiLib.getFormattedAmount(this.freespinTotalValue, true)
      );
...
gWV.findSticky = function () {
  if (
    coreApp.gameModel.userModel.userData.current_round.misc_prizes.count == 0
  ) {
    mValue = coreApp.gameModel.userModel.userData.current_round.screen_wins;
    mValue = this.getNonZeroMultiplierPos(mValue);
  } else {
    let count =
      coreApp.gameModel.userModel.userData.current_round.misc_prizes.count;
    mValue =
      coreApp.gameModel.userModel.userData.current_round.misc_prizes[count - 1]
        .screenWins;
    mValue = this.getNonZeroMultiplierPos(mValue);
  }
  return mValue;
};
gWV.TumbleWin = function (array, num, tickerValues) {
  //parsed values like which symbol name burst and the total number
  if (
    coreApp.gameModel.userModel.userData.current_round.spin_type == "freespin"
  ) {
  ...
  for (
    var j = 0;
    j <
    coreApp.gameModel.obj.current_round.misc_prizes[num].old_reel_symbol.length;
    j++
  ) {
    ...
    if (coreApp.lang == "tr") {
      _mediator.publish(
        ...
        tickerValues[0][0] +
          " X    " +
          "          " +
          pixiLib.getFormattedAmount(array[0]) +
          " " +
          gameLiterals.pays
      );
    } else {
  ...
  _mediator.publish(
    "SHOW_TICKER_MESSAGE",
    winTxt + ": " + pixiLib.getFormattedAmount(this.freespinTotalValue, true)
  );
};
...
gWV.ScatterWin = function () {
  ...
  pixiLib.setText(
    this.ScatterWintxt,
    pixiLib.getFormattedAmount(coreApp.gameModel.obj.current_round.scatter_win)
  );
  _mediator.publish(
    "UpdateWin",
    coreApp.gameModel.obj.current_round.scatter_win
  );
  this.freespinTotalValue =
    this.freespinTotalValue + coreApp.gameModel.obj.current_round.scatter_win;
  var winTxt = gameLiterals.win_text;
  _mediator.publish(
    "SHOW_TICKER_MESSAGE",
    winTxt +
      ": " +
      pixiLib.getFormattedAmount(
        coreApp.gameModel.obj.current_round.scatter_win,
        true
      )
  );
...
gWV.playBonusSymbolAnimation = function (data) {
  if (
    coreApp.gameModel.obj.current_round.scatter_win &&
    coreApp.gameModel.obj.current_round.scatter_win > 0
  ) {
  ...
  if (
    ...
    var animationDuration =
      this.animationConfig[bonusSymbol][0].animationDuration;
    var symbolArray = [];
    this.createWinSymbolContainer();
    ...
    for (var i = 0; i < reelMatrix.length; i++) {
      for (var j = 0; j < reelMatrix[i].length; j++) {
        if (reelMatrix[i][j] === bonusSymbol) {
          ...
          if (
            this.reelConfig.data.eachReelPos &&
            this.reelConfig.data.eachReelPos[i] !== undefined
          ) {
            winSym.x = this.reelConfig.data.eachReelPos[i] + symbolOffset.x;
          } else {
            winSym.x =
              this.symConfig.symbolWidth * i +
              this.symConfig.symbolXGap * i +
              symbolOffset.x;
          }
          winSym.y =
            this.symConfig.symbolHeight * (j + 1) +
            this.symConfig.symbolYGap * j +
            symbolTopPadding +
            symbolOffset.y;
          winSym.playAnimation();
        }
    ...
    if (
      coreApp.gameModel.obj.misc_prizes &&
      coreApp.gameModel.obj.misc_prizes[
        coreApp.gameModel.obj.misc_prizes.count - 1
      ] &&
      coreApp.gameModel.obj.misc_prizes[
        coreApp.gameModel.obj.misc_prizes.count - 1
      ].new_reel
...
gWV.playLandAnim = function (reelId, position) {
  if (
    (this.cacheID != reelId || this.cachePos != position) &&
    position < 6 &&
    position > 0
  ) {
    this.cacheID = reelId;
    ...
    setTimeout(
      function () {
        ...
        if (
          this.reelConfig.data.eachReelPos &&
          this.reelConfig.data.eachReelPos[reelId] !== undefined
        ) {
          winSym.x = this.reelConfig.data.eachReelPos[reelId] + symbolOffset.x;
        } else {
          winSym.x =
            this.symConfig.symbolWidth * reelId +
            this.symConfig.symbolXGap * reelId +
            symbolOffset.x;
        }
        winSym.y =
          this.symConfig.symbolHeight * (position + 1) +
          this.symConfig.symbolYGap * position +
          symbolTopPadding +
          symbolOffset.y;
        winSym.x += 150;
        winSym.y -= 165;
