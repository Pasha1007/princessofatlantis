...
gRStrip.createGameElements = function () {
  ...
  _mediator.subscribe(
    "showExtraSpins",
    this.animateAllScatterOnReel.bind(this)
  );
  _mediator.subscribe("checkAndLand", this.checkAndLand.bind(this));
  _mediator.subscribe(
    "visibilityOfMultiplierSymbol",
    this.visibilityOfMultiplierSymbol.bind(this)
  );
...
gRStrip.createView = function (id) {
  ...
  this.stripData = coreApp.gameController.model.spinData.getReels()[this.reelId]
    ? coreApp.gameController.model.spinData.getReels()[this.reelId]
    : this.stripData;
...
gRStrip.findMultipliers = function () {
  ...
  if (
    ...
    if (coreApp.gameModel.userModel.userData.current_round.misc_prizes != "") {
      ...
      this.totalMultipliers =
        coreApp.gameModel.userModel.userData.current_round.misc_prizes[
          count - 1
        ].screenWins;
      this.totalMultipliers = this.getNonZeroMultiplierPos(
        this.totalMultipliers
      );
    }
  }
...
gRStrip.removeWinSym = function (num) {
  ...
  let temp = [];
  var newarr =
    coreApp.gameModel.obj.current_round.misc_prizes[num].new_symbols.split(";");
  var countstr = 0;
  ...
  for (var i = 1; i < this.symbolsArray.length - 1; i++) {
    for (
      var j = 0;
      j <
      coreApp.gameModel.obj.current_round.misc_prizes[num].old_reel_symbol
        .length;
      j++
    ) {
      ...
      if (
        this.symbolsArray[i].symName ==
        coreApp.gameModel.obj.current_round.misc_prizes[num].old_reel_symbol[j]
      ) {
        if (
  ...
  for (var x = 0; x < currentReelSymbols.length; x++) {
    //creating new symbol.......
    var symb = new ReelSymbol(
      _ng.GameConfig.reelSymbolConfig[currentReelSymbols[x]]
    );
    //change
    ...
    if (i == this.symbolsArray.length - 1) {
      TweenMax.to(
        ...
        _ng.isQuickSpinActive &&
          _ng.quickSpinType == "turbo" &&
          _ng.GameConfig.FastAnim
          ? 0.375 + (this.symbolsPos.length - i) * 0.0375
          : 0.5 + (this.symbolsPos.length - i) * 0.05,
        {
          y: this.symbolsPos[i],
          delay:
            (_ng.isQuickSpinActive && _ng.quickSpinType === "turbo") ||
            (this.forcedSpin && _ng.isForceAllowed)
              ? 0
              : (this.symbolsPos.length - 2 - i) / 20,
...
gRStrip.startSpin = function (argument) {
  ...
  if (_ng.GameConfig.FastAnim) this.speed = 20;
  ...
  if (this.reelConfig.data.reelSpinConfig.startJerk) {
    TweenMax.allTo(
      this.symbolsArray,
      this.reelConfig.data.reelSpinConfig.startJerkSpeed / 1000,
      {
        ...
        onComplete: function () {
          ...
          if (this.reelConfig.data.reelSpinConfig.useBlur) {
            for (var i = 0; i < this.symbolsArray.length; i++) {
              this.symbolsArray[i].changeSymbol(
                this.symbolsArray[i].symName + "_blur"
              );
            }
          }
        }.bind(this),
    ...
    this.spinLoop();
    if (
      this.reelConfig.data.reelSpinConfig.useBlur &&
      !_ng.isQuickSpinActive &&
      _ng.quickSpinType != "turbo"
    ) {
      for (var i = 0; i < this.symbolsArray.length; i++) {
        var symbolImg = this.symbolsArray[i].symName
          ? this.symbolsArray[i].symName
          : this.symbolsArray[i].config.symbol.texture;
  ...
  if (_ng.GameConfig.spineSymbol && !this.isSpining) {
    ...
    for (var i = 0; i < this.symbolsArray.length; i++) {
      if (_ng.isQuickSpinActive && _ng.quickSpinType === "turbo") {
        this.symbolsArray[i].children[0].state.setAnimation(
          0,
          "Spin Loop",
          true
        );
      ...
      this.symbolsArray[i].children[0].state.timeScale =
        _ng.isQuickSpinActive && _ng.quickSpinType === "turbo" ? 2 : 1;
    }
    ...
    if (
      this.symbolsArray[this.symbolsArray.length - 1].y >=
      (this.symConfig.symbolHeight + this.symConfig.symbolYGap) *
        (this.symbolsArray.length - 1)
    ) {
      var bottomSymbol = this.symbolsArray.splice(
        this.symbolsArray.length - 1,
        1
      )[0];
      bottomSymbol.y = 0;
...
gRStrip.setFinalPos = function (argument) {
  ...
  for (let i = symbolsPosLength - 1; i > 0; i--) {
    ...
    const tween = TweenMax.to(this.symbolsArray[i], stopDelay, {
      ...
      delay:
        ...
        (this.forcedSpin && _ng.isForceAllowed)
          ? 0
          : (symbolsPosLength - 2 - i) / 18,
      ease: Linear.easeIn,
      onComplete: function () {
        if (!this.forcedSpin) {
          _ng.isForceAllowed = false;
        }
        if (i == 6) this.playReelStopSound();
  ...
  const checkForceAllowed = setInterval(
    function () {
  ...
  if (this.reelConfig.data.reelSpinConfig.useBlur) {
    ...
    for (let i = 0; i < this.symbolsArray.length; i++) {
      ...
      if (
        this.symbolsArray[i].symName == "s" ||
        this.symbolsArray[i].symName == "scatter"
      ) {
