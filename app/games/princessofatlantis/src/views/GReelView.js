...
view.createView = function (argument) {
  ...
  _mediator.subscribe(
    "scaleReelMaskOnEachReelStop",
    this.updateReelWiseMask.bind(this)
  );
...
view.createFSTopGrid = function (argument) {
  this.fsTopGrid = pixiLib.getElement(
    "Sprite",
    this.reelConfig.fsReelGridTop.image
  );
  this.fsTopGrid.name = "fsTopGrid";
  this.addChild(this.fsTopGrid);
  pixiLib.setProperties(
    this.fsTopGrid,
    this.reelConfig.fsReelGridTop.props["VD"]
  );
...
view.individualStrip = function (spinType, isStopNow) {
  if (spinType == "start") {
    ...
    if (
      this.isAnticipateActive &&
      _ng.GameConfig.AnticipateReq &&
      (!_ng.GameConfig.anticipationReels ||
        _ng.GameConfig.anticipationReels[this.startSpinCounter]) &&
      !isStopNow
    ) {
      ...
      this.reels[this.startSpinCounter].anticipateSpin(
        this.reelsData[this.startSpinCounter],
        isStopNow
      );
      this.currentAnticipationReelId = this.startSpinCounter;
    } else {
      this.reels[this.startSpinCounter].stopSpin(
        this.reelsData[this.startSpinCounter],
        isStopNow
      );
    }
  ...
  if (this.startSpinCounter < this.reelConfig.data.noOfReels) {
    this.delayInterval =
      spinType == "start"
        ? this.reelConfig.data.reelSpinConfig.reelSpinStartGap
        : this.reelConfig.data.reelSpinConfig.reelStopGap;

    var anticipateDelay = this.reelConfig.data.anticipationDelay;
    var delay =
      this.isAnticipateActive && _ng.GameConfig.AnticipateReq
        ? anticipateDelay
        : this.delayInterval;
  ...
  if (
    _ng.GameConfig.aniticipateSymbol &&
    spinType === "stop" &&
    _ng.GameConfig.AnticipateReq &&
    this.isAnticipateActive == false &&
    this.forceStop == false &&
...
view.individualStripStop = function (spinType, isStopNow) {
  ...
  if (
    ...
    _ng.GameConfig.AnticipateReq &&
    (!_ng.GameConfig.anticipationReels ||
      _ng.GameConfig.anticipationReels[this.stopSpinCounter]) &&
    !isStopNow
  ) {
    ...
    //console.log(" Anticipation is activated! ======");
    if (
      !this.isAnticipateSndPlaying ||
      _ng.GameConfig.sounds.playAnticipationForEachReel
    ) {
      _sndLib.stop(_sndLib.sprite.reelSpinning);
      _sndLib.play(_sndLib.sprite.anticipation);
      this.isAnticipateSndPlaying = true;
    ...
    this.reels[this.stopSpinCounter].anticipateSpin(
      this.reelsData[this.stopSpinCounter],
      isStopNow
    );
    ...
    this.reels[this.stopSpinCounter].stopSpin(
      this.reelsData[this.stopSpinCounter],
      isStopNow
    );
  }
  ...
  if (this.stopSpinCounter < this.reelConfig.data.noOfReels) {
    ...
    this.callNextReelInterval = setTimeout(
      this.individualStripStop.bind(this, spinType, isStopNow),
      delay
    );
  } else {
    _mediator.publish("allReelsStarted");
