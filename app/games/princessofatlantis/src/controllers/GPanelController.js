...
pc.addEvents = function () {
  ...
  _mediator.subscribe(
    ...
    this.setUpPanelValues.bind(this)
  );
  _mediator.subscribe(
  ...
  _mediator.subscribe(
    _events.slot.totalBetUpdated,
    this.onTotalBetUpdated.bind(this)
  );
  _mediator.subscribe(
    _events.slot.updateBalance,
    this.updateBalance.bind(this)
  );
  _mediator.subscribe(
    "UPDATE_BALANCE_FROM_DATA",
    this.updateBalance.bind(this)
  );
  ...
  _mediator.subscribe(
    "totalWinShownOnBigWinSettled",
    this.onTotalWinShownOnBigWinSettled.bind(this)
  );
  ...
  _mediator.subscribe(
    "UPDATE_AUTOSPIN_COUNT",
    this.onAutoSpinCountUpdate.bind(this)
  );
  _mediator.subscribe("HIDE_AUTOSPIN_WINDOW", this.onHideAutoSpin.bind(this));
  _mediator.subscribe(
    "updateAutoSpinBtnStates",
    this.updateAutoSpinBtnStates.bind(this)
  );
  //hide gamble button
  _mediator.subscribe(
    "hideGambleButtonOnClick",
    this.hideGambleButtonOnClick.bind(this)
  );
  ...
  _mediator.subscribe(
    "showRealityCheckPopup",
    this.showRealityCheckPopup.bind(this)
  );
  _mediator.subscribe(
    _events.core.error,
    function () {
...
pc.updateTotalBet = function () {
  if (this.popupDisplayIndex == 0) {
    ...
    if (_ng.twoXBetEnabled === true) {
      ...
      _mediator.publish(
        "updatePopupBet",
        totalBet * coreApp.gameModel.spinData.antebet
      );
      _mediator.publish(
        "updateSetBet",
        totalBet * coreApp.gameModel.spinData.antebet
...
pc.onAutoSpinCountUpdate = function (count) {
  if (count) _ng.autoPlayPrevCount = count;
...
pc.updateTwoxbet = function () {
  var totalBet = this.slotModel.getTotalBet();
  _ng.twoXBetEnabled = !_ng.twoXBetEnabled;
  this.view.updateTotalBet(totalBet * coreApp.gameModel.spinData.antebet);
  const doubleChanceONRequest = {
    feature: { toggle: _ng.twoXBetEnabled },
    set: "options",
  };
  ws.send(JSON.stringify(doubleChanceONRequest));
  _mediator.publish(
    "updatePopupBet",
    totalBet * coreApp.gameModel.spinData.antebet
  );
