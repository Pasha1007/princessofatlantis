...
gSC.onGameInit = function (argument) {
    ...
    if (this.model.isFreeSpinActive() || this.model.isReSpinActive() || this.model.isStickyRespinTriggered() || this.model.isStickyRespinActive() || this.model.isFeatureActive() || this.model.isGambleActive()) {
        ...
        _mediator.publish("DisablePanel");
        var unfinishedText = (_ng.GameConfig.unfinishedPopupText) ? _ng.GameConfig.unfinishedPopupText : gameLiterals.unfinished;
...
gSC.dealWithKeyboard = function (e) {
    ...
    if (e.keyCode === 32) {
        ...
        {
            this.checkReelsStoppedKeyUp=setInterval(function(){
                if(this.allReelsStopped)
                {
...
gSC.onAllReelStopped = function () {
    ...
    if (coreApp.gameModel.obj.current_round.spin_type == "freespin"&& coreApp.gameModel.obj.current_round.misc_prizes.count==0) {
     ...
     _mediator.publish("DisablePanel");
        ...
        if(coreApp.gameModel.obj.current_round.misc_prizes.count==0)
            {  
                ...
                if (coreApp.gameModel.isAutoSpinActive()== false && coreApp.gameModel.isFreeSpinActive() == false) {
...
gSC.AllTumbleStepupstep = function (i,winArr) {
    if (i > coreApp.gameModel.obj.current_round.misc_prizes.count || coreApp.gameModel.obj.current_round.misc_prizes == "") {
        this.allReelsStopped = true;
        this.enableMobilePanel();
        return; // Stop recursion when the count is exceeded
    }
    // Calculate the delay for the current step
    let delay = (_ng.isQuickSpinActive && _ng.quickSpinType == "turbo" && _ng.GameConfig.FastAnim) ? 1.35 : 1.5;
    if(i==1)delay=0;
    ...
    TweenMax.to({}, delay, {    
        onComplete: function () {
            ...
            let nextDelay = (_ng.isQuickSpinActive && _ng.quickSpinType == "turbo" && _ng.GameConfig.FastAnim) ? 0.6 : 0.8;
                    ...
                    if (i == coreApp.gameModel.obj.current_round.misc_prizes.count) {
                        TweenMax.to({}, 0.8, {
                            onComplete: function () {
                                ...
                                if (!coreApp.gameModel.isFreeSpinActive() && !coreApp.gameModel.getIsPFSActive()) {
                                    _mediator.publish("enableBuyFeature");
                                }
                            }.bind(this)
                        });
...
gSC.checkWinLimit = function (state) {
    ...
    else if((Number(this.model.spinData.getTotalWinAmount()) >= 5000*(Number(this.model.getTotalBet()))))
        {
...
gSC.onSpinClickHandler = function (isFreeSpin) {
    ...
    _ng.isForceAllowed=true;
    this.allTumbleFinished = false;
    if (coreApp.gameModel.isFreeSpinActive() || coreApp.gameModel.isAutoSpinActive()) {
...
if (!coreApp.gameModel.isFreeSpinActive()) {
    ...
    if(_ng.BuyFSenabled===true){
        ...
        this.model.setBalance(this.model.getBalance() - (this.model.getTotalBet()*coreApp.gameModel.spinData.buyfg) + store);
        if(!coreApp.gameModel.isFreeSpinActive()){
            _mediator.publish(_events.slot.updateBalance);
        }
    }
...
gSC.callNextGameState = function () {
    ...
    if (this.model.isStickyRespinTriggered() || this.model.isStickyRespinActive()) {
        ...
        if (!this.gameStateAnimation) {
            ...
            {
            setTimeout(function () { _mediator.publish("showFreeSpinAwarded"); }, 600);
            }
        }
...
gSC.quickSpinInfoOnSpaceBar = function () {
    ...
    if( _viewInfoUtil.device == "Mobile" || coreApp.gameModel.isAutoSpinActive() ||
        coreApp.gameModel.isFreeSpinActive() || _ng.BuyFSenabled || coreApp.gameModel.getIsPFSActive()) {
        return;
     }
