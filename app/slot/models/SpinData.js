...
spinInfo.saveInitData = function (obj) {
  ...
  if (obj.previous_round) {
      ...
      for (let i = 0; i < obj.previous_round.bonus_details.length; i++) {
        ...
        if (_ng.GameConfig.featureType == "ProgressFeature") {
          if (obj.previous_round.bonus_details[i].type == "progress_bar") {
            ...
            this.current_level =
              obj.previous_round.bonus_details[i].current_level;
          } else if (
            obj.previous_round.bonus_details[i].type == "sticky_wilds_reel"
          ) {
            this.isStickyWildsReelActive = true;
            this.sticky_symbol =
              obj.previous_round.bonus_details[i].sticky_symbol;
            this.positions = obj.previous_round.bonus_details[i].positions;
            this.stickyMsgArr.push(obj.previous_round.bonus_details[i]);
            this.spawningWildReelNum = this.getStickyWildsPositions(
              this.positions
            );
            ...
            obj.previous_round.bonus_details[i].type == "win_both_ways"
          ) {
    ...
    if (obj.previous_round.feature_details) {
      if (
        obj.previous_round.feature_details.type == "200bets" ||
        obj.previous_round.feature_details.type == "ultraspin" ||
        obj.previous_round.feature_details.type == "free_game"
      ) {
        ...
        this.preFeatureActivity =
          this.bonusFreatureMSGType == "200bets" ? true : false;
        this.postFeatureActivity =
          this.bonusFreatureMSGType != "200bets" ? true : false;
      } else {
    ...
    if (
      obj.current_round &&
      obj.current_round.bonus_games_won &&
      obj.current_round.bonus_games_won.length > 0 &&
      obj.current_round.bonus_games_won[0].type == "respins"
    ) {
      this.isStickyRespinTriggered = true;
    }
    ...
    if (
      _ng.GameConfig.featureType == "SWR" &&
      obj.next_round &&
      obj.next_round.type == "respins"
    ) {
      this.spawningWildReelNum = this.getWildsPositions(
        this.formatMatrix(obj.previous_round.matrix)
      );
    ...
    if (obj.next_round.type === "respins" && obj.next_round.spins_left > 0) {
      ...
      if (
        obj.next_round.sticky_positions &&
        obj.next_round.sticky_positions.length > 0
      ) {
        this.isStickyRespinActive = true;
        this.stickyRSSymbols = obj.next_round.sticky_positions;
      }
    ...
    if (obj.next_round && obj.next_round.bonus_game_id) {
      ...
      if (_ng.GameConfig.bonusGames) {
        if (
          Object.keys(_ng.GameConfig.bonusGames).indexOf(
            winObj.bonus_game_id.toString()
          ) > -1
        ) {
          this.isBonusGameTriggerd = true;
          this.featureObject = winObj;
...
spinInfo.saveSpinData = function (obj) {
  ...
  if (
    obj.current_round.post_matrix_info &&
    obj.current_round.post_matrix_info.feature_name
  ) {
  ...
  if (obj.current_round && obj.current_round.bonus_games_won) {
    for (let i = 0; i < obj.current_round.bonus_games_won.length; i++) {
      ...
      if (obj.current_round.bonus_games_won[i].type == "screen_wins") {
        ...
        this.scatterPaySymbol =
          obj.current_round.bonus_games_won[i].screen_symbol;
        this.scatterPayPositions =
          obj.current_round.bonus_games_won[i].num_screen_symbols;
        this.scatterPayAmount = obj.current_round.bonus_games_won[i].win_amount;
      }
      if (
        obj.current_round.bonus_games_won[i].type == "respins" &&
        obj.current_round.bonus_games_won[i].spins_left > 0
      ) {
        this.isRespinActive = true;
      ...
      // Infinity Battle
      if (
        obj.current_round.bonus_games_won[i].type == "200bets" ||
        obj.current_round.bonus_games_won[i].type == "ultraspin" ||
        obj.current_round.bonus_games_won[i].type == "free_game"
      ) {
        this.isBonusFreatureMSG = true;
      ...
      if (
        obj.current_round.bonus_games_won[i].type == "10ScatterCollect" ||
        obj.current_round.bonus_games_won[i].type == "5WinsInARow" ||
        obj.current_round.bonus_games_won[i].type == "3CharacterWin"
      ) {
      ...
      if (
        obj.current_round.bonus_games_won[i].feature ==
      ...
      if (obj.current_round.bonus_games_won[i].type == "sticky_respin") {
        ...
        this.isStickyRespinActive = true;
        this.stickyRSSymbols =
          obj.current_round.bonus_games_won[i].sticky_positions;
      }
      ...
      if (_ng.GameConfig.featureType == "ProgressFeature") {
        if (obj.current_round.bonus_games_won[i].type == "progress_bar") {
          ...
          this.current_level =
            obj.current_round.bonus_games_won[i].current_level;
        } else if (
          obj.current_round.bonus_games_won[i].type == "sticky_wilds_reel"
        ) {
          this.isStickyWildsReelActive = true;
          this.sticky_symbol =
            obj.current_round.bonus_games_won[i].sticky_symbol;
          ...
          obj.current_round.bonus_games_won[i].type == "win_both_ways"
        ) {
          this.isWinBothWaysActive = true;
          this.freespins = obj.current_round.bonus_games_won[i].freespins;
          this.stickyMsgArr.push(obj.current_round.bonus_games_won[i]);
  ...
  if (obj.current_round.post_matrix_info) {
    ...
    if (
      obj.current_round.post_matrix_info.feature_name === "spawning_wild" ||
      obj.current_round.post_matrix_info.feature_name === "random_wild_feature"
    ) {
      this.isSpawningWild = true;
      ...
      if (obj.current_round.post_matrix_info.expand_positions) {
        this.spawningWildReelNum = this.getExpandPositions(
          obj.current_round.post_matrix_info.expand_positions
        );
        this.expandingSymbolInfo =
          obj.current_round.post_matrix_info.expand_positions;
      } else {
        try {
          this.spawningWildReelNum = this.getWildsPositions(
            this.formatMatrix(obj.current_round.post_matrix_info.matrix)
          );
        } catch (e) {}
      }
      ...
      if (obj.current_round.post_matrix_info.random_wild_positions) {
        this.wildPositions =
          obj.current_round.post_matrix_info.random_wild_positions;
      }
    } else if (
      obj.current_round.post_matrix_info.feature_name === "random_symbols" &&
      obj.current_round.post_matrix_info.random_wild_positions.length > 0
    ) {
      this.isSpawningWild = true;
      this.postExpandingMatrix = obj.current_round.post_matrix_info.matrix;
      this.postWildsPositions =
        obj.current_round.post_matrix_info.random_wild_positions;
    ...
    if (obj.current_round.post_matrix_info.feature_name === "walking_wild") {
      ...
      this.postExpandingMatrix = obj.current_round.post_matrix_info.matrix;
      this.walkingWildReelNum = this.getWildsPositions(
        this.formatMatrix(obj.current_round.post_matrix_info.matrix)
      );
    } else {
      this.isWalkingWild = false;
    ...
    if (
      obj.current_round.post_matrix_info.feature_name === "magnum_opus_feature"
    ) {
  ...
  if (this.isRespinActive && _ng.GameConfig.isStickyRespin) {
    ...
    this.isStickyRespinActive = true;
    this.stickyRSSymbols =
      obj.current_round.bonus_games_won[0].sticky_positions;
    this.isRespinActive = false;
  } else if (!this.isRespinActive && _ng.GameConfig.isStickyRespin) {
    this.isStickyRespinTriggered = false;
    this.isStickyRespinActive = false;
  ...
  // multiplier
  if (
    obj.current_round.multipliers &&
    obj.current_round.multipliers.length > 0
  ) {
...
spinInfo.setMatrix = function (obj, postMatrix) {
  ...
  this.paylineWinAmount = obj.current_round.payline_win_amount
    ? obj.current_round.payline_win_amount
    : 0;
  ...
  if (obj.current_round.matrix) {
    ...
    var matrix =
      postMatrix !== undefined ? postMatrix : obj.current_round.matrix;
    this.matrix = matrix;
    var tempMatrix = matrix.split(";");
    this.reels = [];
    for (var i = 0; i < tempMatrix.length; i++) {
    ...
    if (obj.current_round.bonus_games_won) {
      ...
      for (let index = 0; index < bonusObj.length; index++) {
        if (bonusObj[index].type && bonusObj[index].type === "screen_wins") {
          ...
          this.totalLineWins.push({
            line: 0,
            win: bonusObj[index].win_amount,
            blink: scatterPayoutObj[0],
            repeat: 1,
            symbols: scatterPayoutObj[1],
            pos: scatterPayoutObj[2],
          });
        }
      }
    }
  ...
  if (this.totalWin > 0 && obj.current_round.payline_wins.details.length > 0) {
    ...
    for (var i = 0; i < this.totalLineWinsObj.length; i++) {
      ...
      this.totalLineWins.push({
        line: lineObj[0],
        win: lineObj[1],
        blink: lineObj[2].split(""),
        repeat: lineObj[3],
        symbols: lineObj[4].split(""),
        pos: lineObj[5].split(","),
      });
...
spinInfo.setTotalClusterWins = function (obj) {
  ...
  if (this.totalWin > 0 && obj.current_round.cluster_wins.length > 0) {
    ...
    for (var i = 0; i < totalLineWinsObj.length; i++) {
      ...
      this.totalLineWins.push({
        clusterNo: i + 1,
        winAmount: lineObj.win_amount,
        totalSymbols: lineObj.num_symbols,
        symbol: lineObj.symbol,
        symbolPositions: lineObj.symbol_positions,
      });
    }
  }
};
...
spinInfo.setPostMatrix = function () {
  if (this.postMatrixReels) {
    ...
    this.reels =
      this.postMatrixReels.length !== 0 ? this.postMatrixReels : this.reels;
  }
};
...
spinInfo.getWildsPositions = function (reels) {
  var wild = _ng.GameConfig.expandingSymbol
    ? _ng.GameConfig.expandingSymbol
    : "w";
  var ary = [];
